...
if(length(rownames(significance_df))){
  ...
  cl = makeCluster(CPU_num)
  clusterExport(cl, c('bedtools_path'), envir = .GlobalEnv)
  clusterEvalQ(cl, list(library(bedtoolsr), options(bedtools.path = bedtools_path)))
  
  merged_pvalue_df = parApply(cl, significance_df, MARGIN = 1, function(x){
    pvalue = x[3]
    left_bed = read.csv2(x[1], stringsAsFactors = F, header = F, sep = '\t')
    right_bed = read.csv2(x[2], stringsAsFactors = F, header = F, sep = '\t')
    window_df = bt.intersect(a=left_bed, b=right_bed, s=TRUE, wo=TRUE)
  ...
  merged_pvalue_df=do.call('rbind', merged_pvalue_df)
  if(length(rownames(merged_pvalue_df))){
    merged_pvalue_df = merged_pvalue_df[order(merged_pvalue_df$chrm, as.numeric(merged_pvalue_df$start)), ]
  }
  write.table(merged_pvalue_df, file = bed_opt, quote = F, sep="\t", col.names = F, row.names = F)
}else{
  write.table(data.frame(), file = bed_opt, quote = F, sep="\t", col.names = F, row.names = F)
}
